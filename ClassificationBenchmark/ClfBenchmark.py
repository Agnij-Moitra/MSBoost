...
#         X_train, X_test, y_train, y_test = train_test_split(preds.drop("yt", axis=1).sum(axis=1), preds["yt"])
#         final_clfs = LazyClassifier()
#         models, _ = final_clfs.fit(X_train,X_test,y_train,y_test)
#         self.final_estimator = final_clfs.provide_models(X_train,X_test,y_train,y_test)[models['F1 Score'].idxmax()]
...
#         preds_ = self.final_estimator.predict(preds.sum(axis=1))
#         return preds_
class MSBoostClassifier(BaseEstimator, ClassifierMixin):
    """A Gradient Boosting Classifier
    """
